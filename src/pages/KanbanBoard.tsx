...
export default function KanbanBoard() {
  ...
  async function loadTasks() {
    ...
    setTasks(
      enriched.filter(
        (t: any) => String(t.projectId) === String(projectId)
      )
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0b1220] p-6 md:p-10 transition-colors duration-300">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-6">
              <input
                ...
                className="w-full px-5 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800"
              ...
              <div className="grid grid-cols-3 gap-3">
                {PRIORITY_LIST.map((p) => (
                  <button
                    ...
                    className={`py-3 rounded-xl border-2 text-[10px] font-bold uppercase ${
                      priority === p.value
